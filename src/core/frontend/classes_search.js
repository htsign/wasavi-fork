 ...
 * @param {WasaviApp} app
 * @param {WasaviPositionLike} initialPos
 */
...
class SearchUtils {
	...
	constructor(app) {
		 ...
		 * @param {string} quoteChar
		 * @returns {{start: number, end: number} | false}
		 */
		 ...
		 * @param {boolean} [isFindOnly]
		 * @returns {WasaviPosition | false}
		 */
	...
	loop:		do {
					...
					while (true) {
						...
						if (/[.!?]/.test(c)) {
							...
							if (moved === -1 || buffer.isNewline(tpos)
							|| !cpo_J && spaces.test(c)
		...
		/**
		 * @param {number} count
		 * @param {string} [bracketSpecified]
		 * @param {WasaviPosition} [initialPos]
		...
		/**
		 * @param {number} count
		 * @param {string} bigwordChar
		 * @param {boolean} [includeAnchor]
		 * @returns {boolean}
		 */
		function word(count, bigwordChar, includeAnchor) {
			var bigword = bigwordChar == 'W';
		...
		/**
		 * @param {number | WasaviPosition} [p]
		 * @returns {{start: number, end: number, id: number, content: string} | false}
		 */
		function isInTag(p) {
			var text = /** @type {string} */ (app.lastRegexFindCommand.text);
			/** @type {number} */
			var start;
			/** @type {number} */
			var end;
		 ...
		 * @param {number} start
		 * @param {number} end
		 * @returns {{offset: number, matchLength: number} | false}
		 */
...
sequential at:
	...
	#4  0,0  - 9,6   <html> ~ </html>
 */
			var result = /** @type {{isBound(): boolean}} */ (app.low).isBound();
			/** @type {WasaviPosition} */
			var ss;
			/** @type {WasaviPosition} */
			var se;
			if (/** @type {{isBound(): boolean}} */ (app.low).isBound()) {
				// bound marks are guaranteed present once a bound region exists
				ss = /** @type {WasaviPosition} */ (app.marks.getPrivate('<'));
				se = /** @type {WasaviPosition} */ (app.marks.getPrivate('>'));
			}
			else {
			...
			try {
				...
				// offsets come from tag positions located above, so the conversion
				// always resolves to an in-range, non-null position
				/** @type {WasaviPosition} */
				var ss2;
				/** @type {WasaviPosition} */
				var se2;
				if (includeAnchor) {
					ss2 = /** @type {WasaviPosition} */ (buffer.linearPositionToBinaryPosition(
						openTagInfo.offset));
					se2 = /** @type {WasaviPosition} */ (buffer.linearPositionToBinaryPosition(
		...
		function dispatchRangeSymbol(count, targetChar, includeAnchor) {
			switch (targetChar) {
			case '"': case "'": case '`':
				return quote(count, targetChar, includeAnchor);
			...
			case 'w': case 'W':
				return word(count, targetChar, includeAnchor);
		...
		function setParagraphMacros(m) {
			...
			/** @type {string[]} */
			var list = paragraphs = [];
			m.replace(/../g, function (a) {list.push(a); return a});
		}
		/** @param {string} m */
		 ...
		 * @param {string} targetChar
		 * @param {WasaviPosition} [initialPos]
		 * @returns {PairBracketsIndicator | null}
		 */
